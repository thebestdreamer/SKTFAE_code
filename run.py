...
def main():
    ...
    # mask ratio
    mask_ratio = 0.90
    # 注意根据预训练和训练结果的差异调整学习率
    base_learning_rate = 1e-4
    # selecting the dataset and SNR
    data_file = ['rml16a',18]
    ...
    if Mo_train == True:
        ...
        # extracting feature from pretrain model
        get_fea_file(batch_size, mask_ratio, patch_size, data_file, fea_name,label_name,
                     str(num_classes)+'class_MAE_checkpoint/' + model_name, num_classes)
    ...
    sk_cluster(fea_name,num_classes,mask_ratio,patch_size,label_name,center_load=False,need_image=True)
