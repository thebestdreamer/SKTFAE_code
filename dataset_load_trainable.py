...
def cross_entropy_fuzzy(input,target,t,index):
    ...
    Cen_loss = -one_hot_tensor*torch.log(input+1e-6)-0.1*(1-one_hot_tensor)*torch.log(1-input+1e-6)
...
def loss_fun(x,output,fea_mid,alpha=0.1,beta=0.1,num_cluster=4,epoch=0,center=None):
    ...
    Cen_loss = cross_entropy_fuzzy(pos, pseudo_label,P,0)
